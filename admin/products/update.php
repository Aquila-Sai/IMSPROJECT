<?php
// admin/products/update.php - Update Product
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE products SET name=?, brand=?, price=?, description=?, cpu=?, ram=?, storage=?, screen_size=?, stock=?, image_url=? WHERE product_id=?");
    $stmt->bind_param("ssdsssssisi", $name, $brand, $price, $description, $cpu, $ram, $storage, $screen_size, $stock, $image_url, $product_id);
    
    if ($stmt->execute()) {
        $success = "Product updated successfully!";
        // Log admin action
        $admin_id = $_SESSION['user_id'];
        $action = "Updated product: $name (ID: $product_id)";
...
<section class="py-16">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-4xl font-bold logo-glow">Edit Product</h1>
            ...
            <form method="POST" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="cpu" value="<?= $product['cpu'] ?>" required
                               class="w-full px-4 py-2 bg-space-dark border border-neon-blue/30 rounded-lg text-text-base focus:outline-none focus:border-neon-blue">
                    </div>
                    
                    <div>
                        <label class="block text-slate-400 mb-2">RAM *</label>
                        <input type="text" name="ram" value="<?= $product['ram'] ?>" required
                               class="w-full px-4 py-2 bg-space-dark border border-neon-blue/30 rounded-lg text-text-base focus:outline-none focus:border-neon-blue">
                    </div>
                    
                    <div>
                        <label class="block text-slate-400 mb-2">Storage *</label>
                        <input type="text" name="storage" value="<?= $product['storage'] ?>" required
                               class="w-full px-4 py-2 bg-space-dark border border-neon-blue/30 rounded-lg text-text-base focus:outline-none focus:border-neon-blue">
                    </div>
                    
                    <div>
                        <label class="block text-slate-400 mb-2">Screen Size *</label>
                        <input type="text" name="screen_size" value="<?= $product['screen_size'] ?>" required
                               class="w-full px-4 py-2 bg-space-dark border border-neon-blue/30 rounded-lg text-text-base focus:outline-none focus:border-neon-blue">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="image_url" value="<?= $product['image_url'] ?>" required
                           class="w-full px-4 py-2 bg-space-dark border border-neon-blue/30 rounded-lg text-text-base focus:outline-none focus:border-neon-blue">
                    <img src="<?= $product['image_url'] ?>" alt="Preview" class="mt-2 w-32 h-32 object-cover rounded">
                </div>
                
                <div>
                    <label class="block text-slate-400 mb-2">Description *</label>
                    <textarea name="description" rows="5" required
                              class="w-full px-4 py-2 bg-space-dark border border-neon-blue/30 rounded-lg text-text-base focus:outline-none focus:border-neon-blue"><?= $product['description'] ?></textarea>
                </div>
